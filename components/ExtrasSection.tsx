      ...
      {extras.length > 0 && (
        <ul className="flex flex-col gap-2 mt-4">
          {extras.map((ex) => (
            <li
              key={ex.id}
              className="flex items-center justify-between group rounded-lg px-3 py-2.5 bg-[#ff9090]/[0.04] border border-[#ff9090]/10 hover:border-[#ff9090]/25 transition-colors duration-150"
            >
          ...
          <div className="flex justify-between items-center pt-1 px-1">
            ...
            <span className="text-sm font-semibold tabular-nums text-[#ff9090]">+${fmt(total)}</span>
          </div>
        </ul>
      )}
    </div>
