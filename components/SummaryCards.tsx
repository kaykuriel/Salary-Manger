...
export default function SummaryCards({ salary, spent }: SummaryCardsProps) {
  ...
  const barColor = spentPct >= 100 ? "#ff2222" : spentPct >= 80 ? "#ff6600" : "#cc0000";

  return (
    <div className="flex flex-col gap-3">

      {/* Remaining — hero */}
      <div className={`card p-6 ${isOver ? "border-[#ff2222]/30 bg-[#ff2222]/[0.04] hover:border-[#ff2222]/50" : "hover:border-[#3a0000]"}`}>
        <p className="text-xs font-mono uppercase tracking-widest text-[#666] mb-1.5">
          {isOver ? "Over budget by" : "Remaining"}
        </p>
        <p className={`text-5xl font-semibold tabular-nums tracking-tight leading-none ${isOver ? "text-[#ff2222]" : "text-white"}`}>
