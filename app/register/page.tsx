...
export default function RegisterPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (res?.ok) { const { isFirst } = await res.json(); setIsFirst(isFirst); }
    }
    init();
  }, [router]);
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div className="w-full max-w-[360px] flex flex-col gap-6">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-[#555]">{isFirst ? "First account will be admin" : "Join to track your finances"}</p>
        </div>
        ...
        <div className="card p-6 flex flex-col gap-4">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={username} onChange={(e) => { setUsername(e.target.value); clear(); }} placeholder="min. 3 characters" autoComplete="username" autoFocus className="field" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-mono uppercase tracking-widest text-[#555]">Password</label>
              <div className="relative">
                <input type={showPw ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); clear(); }} placeholder="min. 4 characters" autoComplete="new-password" className="field pr-16" />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input type={showConfirm ? "text" : "password"} value={confirm} onChange={(e) => { setConfirm(e.target.value); clear(); }} placeholder="repeat password" autoComplete="new-password" className="field pr-16" />
                <button type="button" onClick={() => setShowConfirm((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#555] hover:text-white text-xs transition-colors select-none" tabIndex={-1}>
        ...
        <p className="text-center text-xs text-[#555]">
          ...
          <a href="/login" className="text-[#cc0000] hover:underline">Sign in</a>
        </p>
      </div>
    </div>
  );
